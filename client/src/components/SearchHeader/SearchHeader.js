import React, { Component } from "react";
import { View, Platform, TouchableOpacity } from "react-native";
import { Text } from "react-native-paper";
import FBStatusBar from "src/components/FBStatusBar";
import AppStyles from "./styles";

import { Appbar, Searchbar } from "react-native-paper";

const TIMEOUT = 240;
export default class SearchHeader extends Component {
  state = {
    _searchQuery: "",
    _isFocused: false,
  };

  onFocus = () => {
    this.setState(
      {
        isFocused: true,
      },
      () => {
        this.props.navigation.navigate("SearchScreen");
        setTimeout(() => {
          this.searchTextInput.focus();
        }, TIMEOUT);
      }
    );
  };

  onBlur = () => {
    this.setState({
      isFocused: false,
    });
  };

  onPress = () => {
    if (this.state._isFocused) {
      this.setState({
        isFocused: false,
      });
      this.props.navigation.pop();
    } else {
      this.onFocus();
    }
  };

  render() {
    const activeScreen = this.props.navigation.state.routes[
      this.props.navigation.state.index
    ].routeName;

    return (
      <View
        style={
          activeScreen === "HomeScreen"
            ? AppStyles.container
            : AppStyles.elevatedContainer
        }
      >
        <FBStatusBar backgroundColor="black" barStyle="light-content" />
        {this.state._isFocused ? (
          <Appbar.Header style={AppStyles.toolbar}>
            <Searchbar
              ref={(input) => {
                this.searchTextInput = input;
              }}
              style={AppStyles.searchbar}
              placeholder="Search"
              icon={
                Platform.OS === "ios" ? "keyboard-arrow-left" : "arrow-back"
              }
              onIconPress={this.onPress}
              onChangeText={(query) => {
                this.setState({ searchQuery: query });
              }}
              onFocus={this.onFocus}
              onBlur={this.onBlur}
              value={this.state._searchQuery}
            />
          </Appbar.Header>
        ) : (
          <Appbar.Header style={AppStyles.toolbar}>
            <Appbar.Action icon="search" onPress={this.onPress} />
            <TouchableOpacity style={AppStyles.btn} onPress={this.onPress}>
              <Text style={AppStyles.btnText}>Search</Text>
            </TouchableOpacity>
            <Appbar.Action icon="face" onPress={this._onSearch} />
          </Appbar.Header>
        )}
      </View>
    );
  }
}
